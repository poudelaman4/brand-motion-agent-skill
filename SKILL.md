...
5. **Apply taste and context:** run the profiler with `--register` and `--frequency`. Drop every vetoed technique, note the register as the reason, read the cliche warnings, and take the duration, gesture, and overshoot ceilings from the taste block. Read `references/taste/brand-register.md`, `references/taste/cliche-and-restraint.md`, and `references/taste/fit-decisions.md`.
